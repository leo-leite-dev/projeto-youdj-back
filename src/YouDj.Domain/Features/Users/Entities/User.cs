...
public sealed class User : EntityBase
{
    ...
    public static User Create(Email email, Username username, string rawPassword,
        DateOfBirth birthDate, Func<string, string> hash)
    {
        if (!birthDate.IsAdult(MinimumDjAge))
            throw new UserException("Idade mínima não permitida.");
    ...
    public void GeneratePasswordResetToken(Func<string> tokenGenerator, TimeSpan duration)
    {
        ...
        if (duration <= TimeSpan.Zero)
            throw new UserException("Duração inválida para o token.");
